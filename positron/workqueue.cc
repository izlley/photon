...
int WorkQueue::workqueue_init(workqueue_t *aWorkqueue, int aNumWorkers, int aNumFreeJobs) {
    int i;
    worker_t *worker;
	job_t *job;
    ...
    // init free job list
    for (i = 0; i < aNumFreeJobs; i++) {
	  if ((job = (job_t *)malloc(sizeof(job_t))) == NULL) {
        perror("Failed to allocate free jobs");
        return 1;
      }
      memset(job, 0, sizeof(*job));
      LL_ADD(job, aWorkqueue->sfreeJob.freeJobs);
	  ++(aWorkqueue->freeCount);
	}
	aWorkqueue->maxFreeCnt = aNumFreeJobs;

    return 0;
}

void WorkQueue::workqueue_shutdown(workqueue_t *aWorkqueue) {
...
void WorkQueue::workqueue_add_job(workqueue_t *aWorkqueue, job_t *aJob) {
...
void WorkQueue::wqFreeJob(workqueue_t *aWorkqueue, job_t *aJob) {
...
void WorkQueue::wqAllocJob(workqueue_t *aWorkqueue, job_t **aJob) {
  int size = 0;
  if (aWorkqueue->freeCount <= 0) {
	*aJob = (job_t *)malloc(sizeof(job_t));
    if (*aJob == NULL)
      return;
  } else {
	pthread_mutex_lock(&aWorkqueue->sfreeJob.freeJobsMutex);
	if (aWorkqueue->freeCount > 0) {
	  ...
	  *aJob = aWorkqueue->sfreeJob.freeJobs;
	  ...
	  pthread_mutex_unlock(&aWorkqueue->sfreeJob.freeJobsMutex);
	  if ((*aJob = (job_t *)malloc(sizeof(job_t))) == NULL)
		return;
	}
  }
}
